...
class Photodiode:
    ...
    def simulate_current(self, skin_light_properties):
        ...
        for light_intensity in waveform:
            ...
            hits = np.random.rand(skin_light_properties.num_photons) < skin_light_properties.hit_probability
            num_hits = np.sum(hits & high_energy_photons)
            num_excited_electrons = min(num_hits, self.num_electrons)
            current_ = num_excited_electrons * 1/self.num_electrons + self.dark_current
...
class PPGSensor:
    ...
    def calculate_SpO2(self, skin_light_properties_red, skin_light_properties_ir):
        # Simulate photodiode current for both red and infrared light
        self.current_red = self.photodiode.simulate_current(skin_light_properties_red)
        self.current_ir = self.photodiode.simulate_current(skin_light_properties_ir)
